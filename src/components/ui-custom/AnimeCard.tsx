...
const AnimeCard = ({ anime }: AnimeCardProps) => {
  ...
  return (
    <div className="glass-card overflow-hidden animate-slide-up group">
      <div className="relative aspect-[3/4] w-full overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex justify-between items-center">
          {anime.score && (
            <div className="flex items-center bg-black/60 backdrop-blur-sm px-2 py-1 rounded-md">
              <Star className="w-4 h-4 text-yellow-400 mr-1" />
              <span className="text-sm font-medium">{anime.score.toFixed(1)}</span>
            </div>
          )}
          <button className="bg-accent/90 hover:bg-accent text-white rounded-md px-3 py-1 text-sm flex items-center gap-1 transition-colors">
